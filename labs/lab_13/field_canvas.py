...
class FieldCanvas(tk.Canvas):
    ...
    def __init__(self, parent: tk.Widget, is_player: bool = True, 
                 click_callback: Optional[Callable[[int, int], None]] = None,
                 hover_callback: Optional[Callable[[int, int], None]] = None) -> None:
        """
        ...
        super().__init__(parent, width=canvas_size, height=canvas_size, 
                        bg=Colors.BG.value, highlightthickness=1, 
    ...
    def _draw_grid(self) -> None:
        ...
        for i in range(FIELD_SIZE + 1):
            # Вертикальные линии
            self.create_line(i * cell_size, 0, i * cell_size, 
                           FIELD_SIZE * cell_size, fill="#CCCCCC", width=1)
            # Горизонтальные линии
            self.create_line(0, i * cell_size, FIELD_SIZE * cell_size, 
                           i * cell_size, fill="#CCCCCC", width=1)
        
        # Подписи координат
        for i in range(FIELD_SIZE):
            # Буквы сверху (A-J)
            letter: str = chr(65 + i)
            self.create_text(i * cell_size + cell_size // 2, -10, 
                           text=letter, font=("Arial", 10, "bold"))
            # Цифры слева (1-10)
            self.create_text(-15, i * cell_size + cell_size // 2, 
                           text=str(i + 1), font=("Arial", 10, "bold"))
    
    def draw_field(self, field: List[List[CellState]], show_ships: bool = True, 
                   highlight_adjacent: bool = True, game_started: bool = False) -> None:
        """
    ...
    def _get_cell_color(self, state: CellState, show_ships: bool, game_started: bool) -> str:
        """Возвращает цвет клетки в зависимости от состояния"""
    ...
    def _highlight_adjacent_cells(self, field: List[List[CellState]]) -> None:
        """Выделяет серым соседние клетки от уничтоженных кораблей"""
        cell_size: int = GUISize.CELL_SIZE.value
                            ...
                            if 0 <= nx < FIELD_SIZE and 0 <= ny < FIELD_SIZE:
                                if field[ny][nx] == CellState.MISS:
                                    ...
                                    self.create_rectangle(x1, y1, x2, y2, 
                                                        fill=Colors.GRAY_MARK.value, 
                                                        outline=Colors.GRAY_MARK.value, 
                                                        tags="cell")
    
    def draw_ship_preview(self, ship: Ship, x: Optional[int], y: Optional[int], valid: bool = True) -> None:
        """
        ...
        for cx, cy in coords:
            if 0 <= cx < FIELD_SIZE and 0 <= cy < FIELD_SIZE:
                ...
                self.create_rectangle(x1, y1, x2, y2, fill=color, 
                                    outline=color, tags="preview", 
                                    stipple="gray50")
    
    def highlight_cell(self, x: int, y: int) -> None:
        """Подсвечивает клетку при наведении"""
        ...
        if x is not None and y is not None:
            if 0 <= x < FIELD_SIZE and 0 <= y < FIELD_SIZE:
                ...
                self.create_rectangle(x1, y1, x2, y2, fill="", 
                                    outline=Colors.HOVER.value, width=3, 
                                    tags="highlight")
